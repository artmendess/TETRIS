...
class Text:
    ...
    def draw(self):
        ...
        self.font.render_to(self.app.screen, (WIN_W * 0.64, WIN_H * 0.8), 
                            ...
                            size=TILE_SIZE * 1.8)
        self.font.render_to(self.app.screen, (WIN_W * 0.64, WIN_H * 0.9), 
                    text=f'High: {self.app.tetris.highscore}', fgcolor='white', 
                    size=TILE_SIZE * 1.0)
...
class Tetris:
    ...
    def put_tetromios_blocks_in_array(self):
    ...
    def check_tetromino_landing(self):
        if self.tetromino.landing:
            if self.is_game_over():
                ...
                self.speed_up = False
                self.put_tetromios_blocks_in_array()
...
def salvar_score(score, caminho="score.json"):
    scores = carregar_scores(caminho)
    scores.append(score)
    with open(caminho, "w") as f:
        json.dump(scores, f)

def carregar_scores(caminho="score.json"):
    if os.path.exists(caminho):
        with open(caminho, "r") as f:
            return json.load(f)
    return []

def carregar_highscore(caminho="score.json"):
    if os.path.exists(caminho):
        with open(caminho, "r") as f:
            scores = json.load(f)
